	...
	function modificar_inscripciones($numinscripcion,$numcurso,$aniogestion,$niveleducativo)
	{
		...
		$datosgestion=$this->modelo_gestion->buscar_gestion($aniogestion);
		 ...
		 $datospension=$this->modelo_pensiones->buscar_pension($numinscripcion);	
			...
			$band=0;  //para verificar si tiene alguna pensino ya pagada
			
			if($pension1=="pagado") { $band++; }
			if($pension2=="pagado") { $band++; }
			if($pension3=="pagado") { $band++; }
			if($pension4=="pagado") { $band++; }
			if($pension5=="pagado") { $band++; }
			if($pension6=="pagado") { $band++; }
			if($pension7=="pagado") { $band++; }
			if($pension8=="pagado") { $band++; }
			if($pension9=="pagado") { $band++; }
			if($pension10=="pagado"){ $band++; }
